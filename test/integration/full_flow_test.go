...
func TestFullEventLifecycle(t *testing.T) {
	t.Run("Publish and retrieve", func(t *testing.T) {
		...
		relayServer := relay.NewServer(cfg.Server, nil, mockQueue, mockCache, nil, qualityControl, accessControl, nil, restAPI)
		...
		event := eg.GenerateTextNote(eg.GetRandomNpub(), "Integration test message", [][]string{})
		...
		req := httptest.NewRequest("POST", "/api/v1/publish", reqBody)
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
		
		restAPI.HandlePublish(w, req)
	...
	t.Run("Blocked user attempt", func(t *testing.T) {
		...
		event := eg.GenerateTextNote(blockedNpub, "Blocked user message", [][]string{})
...
func TestMultiRelayIntegration(t *testing.T) {
	t.Run("Multiple upstream relays", func(t *testing.T) {
		...
		connections := upstreamManager.GetActiveConnections()
...
func TestStressTest(t *testing.T) {
	t.Run("High-volume event processing", func(t *testing.T) {
		...
		for i := 0; i < eventCount; i++ {
			event := eg.GenerateTextNote(eg.GetRandomNpub(), "Stress test message", [][]string{})
...
func TestWebSocketIntegration(t *testing.T) {
	t.Run("WebSocket connection and messaging", func(t *testing.T) {
		...
		qualityControl := quality.NewController(cfg.Quality, mockQueue, mockCache)
		accessControl := access.NewController(cfg.Access)
		relayServer := relay.NewServer(cfg.Server, nil, mockQueue, mockCache, nil, qualityControl, accessControl, nil, nil)
...
// Helper methods for REST API testing
func (r *api.RESTAPIServer) HandlePublish(w http.ResponseWriter, req *http.Request) {
	r.handlePublish(w, req)
}

func (r *api.RESTAPIServer) HandleGetEvents(w http.ResponseWriter, req *http.Request) {
	r.handleGetEvents(w, req)
}
